...
export const create = async (req, res) => {
    try {
        ...
        broadcast('DEVOTEE_CREATED', { ...devoteeBody, id: result.id || devoteeBody.id });
        res.json({ success: true, ...result });
    } catch (error) {
        res.status(500).json({ success: false, error: error.message });
    }
...
export const update = async (req, res) => {
    try {
        ...
        broadcast('DEVOTEE_UPDATED', { id: req.params.id, ...devoteeBody });
        res.json({ success: true, ...result });
    } catch (error) {
        res.status(500).json({ success: false, error: error.message });
    }
...
export const remove = async (req, res) => {
    try {
        ...
        broadcast('DEVOTEE_DELETED', { id: req.params.id });
        res.json({ success: true, ...result });
    } catch (error) {
        res.status(500).json({ success: false, error: error.message });
    }
